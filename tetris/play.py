import keyboard
import tetris.display
import json
from threading import Thread
from tetris.display import format, theme, sizes
...
class Game:
    ...
    def login(self):
        screen = tetris.display.Screen()
        win = tetris.display.Window(size=[18, 6])
    ...
    def menu(self):
        screen = tetris.display.Screen()
        win = tetris.display.Window(size=[18, 9])
    ...
    def load_game(self):
        ...
        game = tetris.display.Window(size=sizes[self.mode])
        game.set_shape(shape=tetris.display.Shape(), pos=("m", "u"))
        ...
        preview = tetris.display.Window(size=[17, 6], pos=(
            game.pos[0]+game.size[0]+4, game.pos[1]))
        preview.set_shape(shape=tetris.display.Shape(
    ...
    def new_game(self):
        ...
        game = tetris.display.Window(size=sizes[self.mode])
        game.set_shape(shape=tetris.display.Shape(
            names=tetris.display.shape_names[self.mode]), pos=("m", "u"))
        self.main_win = game

        preview = tetris.display.Window(size=[17, 6], pos=(
            game.pos[0]+game.size[0]+4, game.pos[1]))
        preview.set_shape(shape=tetris.display.Shape(
            names=tetris.display.shape_names[self.mode]), pos=("m", "m"))
        self.next_win = preview
    ...
    def run(self):
        ...
        help_win = tetris.display.Window(size=(17, 7), pos=(
        ...
        while True:
            ...
            if not self.main_win.can_move():
                ...
                if not self.main_win.game_over():
                    self.main_win.set_shape(shape=tetris.display.Shape(
                        names=(self.next_win.shape.name,)), pos=("m", "u"))
                    self.main_win.shape.color = self.next_win.shape.color
                    self.next_win.set_shape(tetris.display.Shape(
                        names=tetris.display.shape_names[self.mode]), pos=("m", "m"))
                else:
                    break
